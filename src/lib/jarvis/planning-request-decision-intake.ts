...
export function looksLikePlanningRequestDecision(question: string) {
  return /terminwunsch/i.test(question) && /(freigeb|gib[\s\S]{0,80}\bfrei\b|genehmig|bestätig|bestaetig|ablehn|zurückweis|zurueckweis)/i.test(question);
}

export function extractPlanningRequestDecision(question: string): PlanningRequestDecisionIntake {
  const decision = /(ablehn|zurückweis|zurueckweis)/i.test(question)
    ? "reject"
  ...
  const explicitId = /^(?:freigeben|genehmigen|bestätigen|bestaetigen|ablehnen|zurückweisen|zurueckweisen)$/i.test(explicitCandidate)
    ? ""
    : explicitCandidate;
