...
const MAIN_AREAS: JarvisSystemArea[] = [
  ...
  moduleArea("salesJournal", "Sales-Journal", "Tatsächlich ausgeführte Vertriebsaktivitäten chronologisch und ohne eigene Aufgaben- oder Zeiterfassung dokumentieren.", ["eigene Aktivitäten prüfen", "Telefonat, E-Mail, Kundentermin oder Angebotsaktivität mit Kunde und kurzer Notiz erfassen", "als Geschäftsführung nach Mitarbeitern filtern"], ALL_INTERNAL_ROLES, ["vertriebstagebuch", "sales tagebuch", "vertriebsaktivitäten"]),
...
const SYSTEM_SERVICE_AREAS: JarvisSystemArea[] = [
  area({
    id: "system.prospectLifecycle",
    label: "Interessenten und Sales-Journal",
    kind: "system_service",
    keywords: ["Interessent", "Interessent umwandeln", "Sales-Journal", "Vertriebsaktivität", "Vertriebstagebuch"],
    purpose: "Interessenten ohne vorzeitige Kundennummer im bestehenden CRM führen und ausgeführte Vertriebsaktivitäten rollenbegrenzt dokumentieren.",
    workflows: [
      "Interessenten ohne Kundennummer anlegen und im Sales-Journal begleiten",
      "Projekt- und Angebotsanlage bis zur bewussten Kundenübernahme sperren",
      "bei der Umwandlung genau einmal eine Kundennummer vergeben",
      "Vertriebsaktivitäten von Aufgaben und Zeitbuchungen getrennt halten",
    ],
    ...
    verification: {
      ...
      sourceRefs: [
        ...
        "src/app/api/sales-journal/route.ts",
        "src/lib/projects/project-contact-eligibility.ts",
        "src/lib/jarvis/current-product-guidance.ts",
      ],
    },
  }),
  area({
    id: "system.projectCreationConfirmation",
    label: "Hinweise vor Projektanlage",
    kind: "system_service",
    keywords: ["Kundenhinweis vor Projektanlage", "Projekthinweis bestätigen", "Projektanlage Bestätigung"],
    purpose: "Markierte Kunden- und Projekthinweise vor der ersten Speicherung eines neuen Projekts sichtbar und nachweisbar bestätigen lassen.",
    workflows: [
      "Projektmaske vollständig ausfüllen",
      "relevante Kunden- und Projekthinweise vor persistProject anzeigen",
      "bei Zurück die unveränderte Maske offen lassen",
      "Bestätigung erst nach der Anlage mit der neuen Projekt-ID protokollieren",
    ],
  ...
  area({
    id: "system.hourlyRecurringBilling",
    label: "Stunden-Dauerläufer Abrechnung",
    kind: "system_service",
    keywords: ["Stunden-Dauerläufer", "Stundenentwurf", "Tagesnachweis", "Kundentext", "Hybrid-Forecast"],
    purpose: "Stunden-Dauerläufer aus belegten Zeiten, Abrechnungsleistungen, Tagesnachweisen und bestätigter Monatsplanung nachvollziehbar abrechnen und prognostizieren.",
    workflows: [
      "Live- und manuelle Zeiten demselben Monatsentwurf zuordnen",
      "interne Stempelungen je Leistungstag kundenwirksam zusammenfassen",
      "vollständigen Kundentext vor der Fakturierung verlangen",
      "Entwurf und noch offene bestätigte Planung ohne Doppelzählung forecasten",
    ],
    roles: FINANCE_ROLES,
    verification: {
      status: "verified",
      checkedAt: "2026-08-10",
      sourceRefs: [
        "src/lib/time/stamp-session-billing-service.ts",
        "src/lib/invoices/hourly-billing-details.ts",
        "src/lib/invoices/hourly-customer-text-service.ts",
        "src/lib/forecast/hourly-recurring.ts",
        "src/lib/jarvis/current-product-guidance.ts",
      ],
    },
  }),
  area({
    id: "system.netWorkDuration",
    label: "Nettoarbeitszeit und Pausen",
    kind: "system_service",
    keywords: ["Nettoarbeitszeit", "Pause doppelt abziehen", "Pausenlogik", "Soll Ist Zeit"],
    purpose: "Gespeicherte Dauer als kanonische Nettoarbeitszeit behandeln und Pausen ausschließlich als zusätzlichen Nachweis ausweisen.",
    workflows: [
      "Nettoarbeitszeit aus dem kanonischen durationMs lesen",
      "pauseMs niemals ein zweites Mal abziehen",
      "fehlende geplante Pause transparent bestätigen lassen",
      "Zeitstatus als Soll-Nettozeit gegen Ist-Nettozeit erklären",
    ],
    ...
    verification: {
      ...
      sourceRefs: [
        "src/lib/time/work-duration.ts",
        "src/lib/time/project-time-entry-service.ts",
        "src/lib/time/stamp-session-stop-service.ts",
        "src/lib/jarvis/current-product-guidance.ts",
      ],
    },
  }),
  area({
    id: "system.projectContractMonths",
    label: "Projektlaufzeit und Leistungsmonate",
    kind: "system_service",
    keywords: ["Projektlaufzeit", "Vertragsmonat", "Leistungsmonat", "außerhalb der Laufzeit", "halbjährlich"],
    purpose: "Dauerläufermonate an der Projektlaufzeit begrenzen und Fakturierungsintervalle nur in ihrer tatsächlich implementierten Wirkung erklären.",
    workflows: [
      "Leistungsmonate innerhalb der Vertragslaufzeit prüfen",
      "monatlichen Status eines Stunden-Dauerläufers getrennt bewerten",
      "halbjährlich als Projektstammwert speichern",
      "keine noch nicht implementierte Sechsmonatsautomatik behaupten",
    ],
    roles: REPORT_NAVIGATION_ROLES,
    verification: {
      status: "verified",
      checkedAt: "2026-08-10",
      sourceRefs: [
        "src/lib/projects/project-month-range.ts",
        "src/lib/forecast/hourly-recurring.ts",
        DASHBOARD_SOURCE,
        "src/lib/jarvis/current-product-guidance.ts",
      ],
