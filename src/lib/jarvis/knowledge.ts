...
export function resolveJarvisStorageGuidance(
  question: string
): JarvisHelpResult | undefined {
  ...
  const hasStorageSignal =
    ...
    /\b(?:wo|wie)\b.*\b(?:bilder|dateien|dokumente|pdfs?|xrechnungen?)\b.*\b(?:gespeichert|abgelegt|geladen|geholt)\b/.test(
      normalized
    ) ||
    /\b(?:gespeichert\w*|ausgelager\w*)\b.*\b(?:pdfs?|dateien?|dokumente?)\b/.test(
      normalized
    );
  if (!hasStorageSignal) return undefined;

  if (
    /\b(?:access key|secret|zugangsschlussel|schlussel|passwort|token|credentials?|anmeldedaten)\b/.test(
      normalized
    )
  ) {
    return {
  ...
  if (/\b(?:pwa|mobile|app|api|schnittstelle)\b/.test(normalized)) {
    return {
      type: "answer",
  ...
  if (
    /\b(?:welche|was fur|umfang|dateityp|dateifamil|alles|betroffen)\b/.test(normalized) &&
    /\b(?:bild|datei|dokument|pdf|rechnung|angebot|bericht)\w*\b/.test(normalized)
  ) {
    return {
      type: "answer",
  ...
  if (/\b(?:code|technisch|klasse|dateipfad|route|zusammenhang|implement|ablauf)\b/.test(normalized)) {
    return {
      type: "answer",
  ...
  if (/\b(?:auswertung\w*|kennzahl\w*|umsatz\w*|offene posten|forecast|statistik\w*|reporting)\b/.test(normalized)) {
    return {
      type: "answer",
  ...
  if (/\b(?:langsam|performance|ladezeit|laden|cache|etag|skalier|viele dateien)\b/.test(normalized)) {
    return {
      type: "answer",
  ...
  if (/\b(?:fehler|ausfall|nicht erreichbar|fallback|crash|rollback|prufsumme|kaputt|konsistent)\b/.test(normalized)) {
    return {
      type: "answer",
  ...
  if (/\b(?:losch|gelo|storno|aufbewahr|retention|audit|archiv)\w*\b/.test(normalized)) {
    return {
      type: "answer",
  ...
  if (/\b(?:altdatei|migration|migrier|mirror|switch|bestand)\w*\b/.test(normalized)) {
    return {
      type: "answer",
