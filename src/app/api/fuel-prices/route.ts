...
let cache: { expiresAt: number; value: FuelPricePayload } | null = null;
...
function unavailablePayload(configured: boolean, message: string): FuelPricePayload {
  return {
    configured,
    status: configured ? "unavailable" : "not_configured",
    source: "Tankerkönig / Markttransparenzstelle für Kraftstoffe (MTS-K)",
    station: {
      id: "",
      name: "HERM in Buchen",
...
async function loadFuelPrices(): Promise<FuelPricePayload> {
  ...
  try {
    ...
    if (!data.ok) throw new Error(data.message || "API-Antwort war nicht erfolgreich.");

    const stations = data.station ? [data.station] : Array.isArray(data.stations) ? data.stations : [];
    const station =
      stations.find(
        (candidate) =>
    ...
    if (!station) throw new Error("Die konfigurierte HERM-Tankstelle wurde nicht gefunden.");

    const fetchedAt = new Date().toISOString();
    const value: FuelPricePayload = {
      configured: true,
      status: "live",
      source: "Tankerkönig / Markttransparenzstelle für Kraftstoffe (MTS-K)",
      station: {
      ...
      prices: {
        diesel: typeof station.diesel === "number" && station.diesel > 0 ? station.diesel : null,
        e5: typeof station.e5 === "number" && station.e5 > 0 ? station.e5 : null,
        e10: typeof station.e10 === "number" && station.e10 > 0 ? station.e10 : null,
      },
      fetchedAt,
    ...
    if (cache?.value.status === "live") {
      return {
        ...cache.value,
        status: "unavailable",
        message: "Die Preisquelle ist vorübergehend nicht erreichbar. Der zuletzt geladene Preis wird angezeigt.",
      };
    }
    return unavailablePayload(
      true,
...
export async function GET(req: Request) {
  ...
  if (!canViewCustomerRevenueAnalytics(actorResult.actor)) {
    return NextResponse.json({ error: "Du darfst Kraftstoffpreise nicht einsehen." }, { status: 403 });
  }

  return NextResponse.json(await loadFuelPrices());
}
