...
export async function GET(req: Request) {
  ...
  const calculatePeriod = (targetPeriod: RevenuePeriodInput) => ({
    customerRevenue: calculateCustomerRevenueMix({ invoices, projects, contacts, period: targetPeriod }),
    additionalSales: calculateAdditionalSalesRevenueMix({
      invoices,
      offers,
      potentials,
      taskLinks,
  ...
  const response: CustomerRevenueAnalyticsResponse = {
    period,
    previousPeriod,
    current: calculatePeriod(period),
    previous: calculatePeriod(previousPeriod),
    dataQuality: {
      legacyInvoiceCount,
