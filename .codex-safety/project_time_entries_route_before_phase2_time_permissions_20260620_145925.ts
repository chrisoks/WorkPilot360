...
function canManageProjectTimeEntries(role: Role) {
  return (
    role === Role.ADMIN ||
...
function canApproveOvertime(role: Role) {
...
export async function POST(req: Request) {
  ...
  const actorCanManageProjectTime = canManageProjectTimeEntries(actor.role);
  const actorCanApproveOvertime = canApproveOvertime(actor.role);
