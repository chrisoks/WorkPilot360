...
function partyXml(role: "AccountingSupplierParty" | "AccountingCustomerParty", party: {
  ...
  const contactName = party.contactName || party.name;
  const contactEmail = party.contactEmail || party.endpoint || "";
  return [
    `  <cac:${role}>`,
    "    <cac:Party>",
    ...
    ...(role === "AccountingSupplierParty"
      ? [
          "      <cac:Contact>",
          `        <cbc:Name>${escapeXml(contactName)}</cbc:Name>`,
          ...(party.contactPhone ? [`        <cbc:Telephone>${escapeXml(party.contactPhone)}</cbc:Telephone>`] : []),
          ...(contactEmail ? [`        <cbc:ElectronicMail>${escapeXml(contactEmail)}</cbc:ElectronicMail>`] : []),
          "      </cac:Contact>",
        ]
      : []),
    "    </cac:Party>",
