...
async function getInvoiceBuyerReference(organizationId: string, projectId: string) {
  ...
  const rows = await prisma.$queryRaw<Array<{ leitwegId: string | null }>>`
    ...
    LIMIT 1
  `;
  return cleanText(rows[0]?.leitwegId);
}
...
async function getXRechnungAttachment(organizationId: string, invoiceId: string, documentNumber: string) {
  const invoiceRows = await prisma.$queryRaw<InvoiceMailRow[]>`
    SELECT "id", "projectId", "invoiceNumber", "status", "customerName", "customerStreet", "customerCity",
  ...
  const xrechnungInvoice = {
    ...
    paymentTermDays,
    buyerReference: await getInvoiceBuyerReference(organizationId, invoice.projectId),
  };
...
export async function POST(req: Request) {
  ...
  if (kind === "invoice" && eInvoiceFormat === "zugferd") {
    return NextResponse.json(
      { error: "ZUGFeRD ist fachlich vorbereitet, aber technisch noch nicht fuer den Versand angebunden." },
      { status: 400 }
    );
  }

  const shouldAttachStoredPdf = Boolean(body.attachPdf) || (kind === "invoice" && eInvoiceFormat === "pdf-xrechnung");
