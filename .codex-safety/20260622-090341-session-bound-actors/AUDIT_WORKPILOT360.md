...
## Phase-2-Fortsetzung: Benachrichtigungen und News-Interaktionen

Stand: 2026-06-20

Umgesetzt:
- News-Sichtbarkeitspruefung in `src/lib/news-feed/visibility.ts` zentralisiert.
- `src/app/api/news-feed/route.ts` nutzt die gemeinsame Sichtbarkeitsregel beim Laden und beim Gelesen-Markieren.
- `src/app/api/news-feed/comments/route.ts`, `reactions/route.ts` und `votes/route.ts` pruefen vor dem Schreiben, ob der aktive Benutzer den Beitrag sehen darf.
- `src/app/api/notifications/route.ts` verlangt fuer das Erzeugen neuer Benachrichtigungen jetzt einen aktiven Actor mit zentraler Berechtigung.
- `src/components/dashboard/dashboard-page.tsx` sendet beim automatischen Abrechnungsbereit-Hinweis den aktiven Benutzer mit und startet den Aufruf nicht ohne aktive Benutzerkennung.

Rollen:
- Eigene Benachrichtigungen lesen und als gelesen markieren: weiterhin der jeweilige aktive Benutzer.
- Benachrichtigungen aktiv erzeugen: Admin, Geschaeftsfuehrung, Fuehrungskraft und Buchhaltung.
- News lesen/interagieren: aktive Benutzer nur innerhalb der jeweiligen Sichtbarkeit des Beitrags.

Bewusst nicht geaendert:
- News-Beitraege duerfen weiterhin von aktiven Benutzern erstellt werden. Dieser Block schraenkt zuerst die Sichtbarkeit und Interaktion ab, ohne den internen News-Workflow fachlich zu verengen.
- Direkte Systembenachrichtigungen aus anderen APIs bleiben in ihren jeweiligen Fachrouten. Der freie Sammel-Endpunkt `/api/notifications` wurde abgesichert.

Warum:
- News-Kommentare, Reaktionen, Abstimmungen und Gelesen-Status duerfen nicht auf Beitraege geschrieben werden, die fuer den Benutzer unsichtbar sind.
- Der freie Notification-POST kann Meldungen an andere Benutzer erzeugen und braucht deshalb einen nachvollziehbaren Actor plus Rollenbremse.
- Die bestehende automatische Abrechnungsbereit-Meldung bleibt nutzbar, aber nur noch aus berechtigten Rollen.

Checks:
- `git diff --check -- src/lib/permissions/index.ts src/lib/news-feed/visibility.ts src/app/api/notifications/route.ts src/app/api/news-feed/route.ts src/app/api/news-feed/comments/route.ts src/app/api/news-feed/reactions/route.ts src/app/api/news-feed/votes/route.ts src/components/dashboard/dashboard-page.tsx`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`

Naechster sinnvoller Schritt:
- Abwesenheiten/Planungsbenachrichtigungen oder Ideen-/Content-Restwege erneut aus Phase 2-Sicht pruefen, je nachdem ob wir weiter operative Workflows oder Altbereiche priorisieren.

## Phase-2-Fortsetzung: Abwesenheiten und Planungsbenachrichtigungen

Stand: 2026-06-20

Umgesetzt:
- Abwesenheits-Manager-Regel in `src/lib/permissions/index.ts` zentralisiert.
- `src/app/api/absences/route.ts` verlangt jetzt auch beim Lesen einen aktiven Actor.
- Abwesenheits-Schreibwege nutzen die zentrale Manager-Regel statt einer lokalen Rollenpruefung.
- Vertreter, Zielbenutzer und Benachrichtigungsempfaenger werden auf aktive Benutzer begrenzt.
- `src/app/api/planning-entries/route.ts` verlangt jetzt auch beim Lesen einen aktiven Actor.
- Planungsbenachrichtigungen pruefen vorhandene Duplikate jetzt organisationsgebunden.
- `src/components/dashboard/dashboard-page.tsx` sendet beim Laden von Abwesenheiten und Planung den aktiven Benutzer mit.

Rollen:
- Abwesenheiten fuer andere verwalten/final bearbeiten: Admin, Geschaeftsfuehrung und Fuehrungskraft.
- Eigene Abwesenheiten eintragen/bearbeiten/loeschen: bleibt fuer den jeweiligen aktiven Benutzer moeglich.
- Planung verwalten: bleibt Admin, Geschaeftsfuehrung und Fuehrungskraft.
- Eigene Terminwuensche: bleiben fuer aktive Benutzer moeglich, sofern sie als `requested` gespeichert werden.

Bewusst nicht geaendert:
- Die Leseanzeige fuer Abwesenheiten und Planungsboard wurde noch nicht fachlich nach Rolle eingeschraenkt. Dieser Block schliesst zuerst anonyme/ungueltige API-Zugriffe, ohne Kalender- und Board-Workflows zu verengen.
- Bestehende Abwesenheits- und Planungsbenachrichtigungslogik bleibt fachlich erhalten.

Warum:
- Abwesenheiten und Planungen enthalten personenbezogene Zeit-, Vertretungs- und Terminangaben.
- Vorher waren die Schreibwege weitgehend actor-basiert, die Lesewege aber noch ohne Actor erreichbar.
- Planungsbenachrichtigungen duerfen bei Dublettenpruefungen nicht organisationsuebergreifend wirken.

Checks:
- `git diff --check -- src/app/api/absences/route.ts src/app/api/planning-entries/route.ts src/components/dashboard/dashboard-page.tsx src/lib/permissions/index.ts`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`

Naechster sinnvoller Schritt:
- Ideen-/Content-Restwege als Altbereich gezielt pruefen und entscheiden, ob sie weiter abgesichert oder nur als deaktivierter Legacy-Bereich dokumentiert werden.

## Phase-2-Fortsetzung: Ideen-, Content- und Abrechnungswarnungs-Restwege

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/idea-store/route.ts` verlangt jetzt fuer Lesen, Erstellen und Interaktionen einen aktiven Actor. Der alte Demo-Fallback wurde entfernt.
- Gaeste duerfen den Ideenbereich nicht verwenden; Ideen anheften duerfen nur Admin, Geschaeftsfuehrung und Fuehrungskraft.
- `src/app/api/content-items/route.ts` verlangt jetzt auch beim Lesen einen aktiven Actor. Schreiben, Statuswechsel und Loeschen sind auf Admin, Geschaeftsfuehrung und Fuehrungskraft begrenzt.
- `src/app/api/unbilled-time-alerts/route.ts` nutzt eine zentrale Berechtigung fuer offene Abrechnungszeit-Warnungen. Lesen und Ausloesen sind auf Admin, Geschaeftsfuehrung, Fuehrungskraft und Buchhaltung begrenzt.
- `src/components/dashboard/dashboard-page.tsx` sendet beim Laden des Content-Altbereichs den aktiven Benutzer mit und bricht Ideen-/Content-Aktionen sauber ab, wenn kein aktiver Benutzer bestimmbar ist.

Rollen:
- Ideenbereich lesen und normal interagieren: aktive Benutzer ausser Gast.
- Ideen anheften: Admin, Geschaeftsfuehrung und Fuehrungskraft.
- Content-Altbereich verwalten: Admin, Geschaeftsfuehrung und Fuehrungskraft.
- Offene Abrechnungszeit-Warnungen sehen/ausloesen: Admin, Geschaeftsfuehrung, Fuehrungskraft und Buchhaltung.

Bewusst nicht geaendert:
- Der Content-Management-Bereich bleibt fachlich als Altbereich/deaktivierter UI-Bereich dokumentiert und wurde nicht wieder ausgebaut.
- Operative PDF-/Berichtswege wie Arbeitsberichte, Abschlusspruefungen, Rauchmelderberichte, Winterdienstlaeufe und Stundenmetriken wurden erneut quergeprueft, aber nicht fachlich weiter verengt, weil sie bereits Actor-/Organisationsschutz besitzen und teils fachliche Spezialablaeufe bedienen.

Warum:
- Ideen, Content und Abrechnungswarnungen sind keine oeffentlichen Endpunkte und duerfen nicht ueber Demo-Fallbacks oder fehlende Actor-Pruefung erreichbar sein.
- Der Content-Bereich wird im UI kaum noch genutzt, sollte aber trotzdem serverseitig nicht offen bleiben.
- Abrechnungswarnungen koennen Benachrichtigungen und fachliche Folgeprozesse ausloesen und brauchen deshalb dieselbe Rollenbremse fuer Lesen und Ausloesen.

Checks:
- `git diff --check -- src/lib/permissions/index.ts src/app/api/idea-store/route.ts src/app/api/content-items/route.ts src/app/api/unbilled-time-alerts/route.ts src/components/dashboard/dashboard-page.tsx`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`

Naechster sinnvoller Schritt:
- Finaler Phase-2-Quercheck der Restliste und danach Abschlussdokumentation: welche aktiven Wege abgesichert sind, welche Bereiche bewusst Legacy bleiben und welche optionalen Produktentscheidungen spaeter offen sind.

## Phase-2-Abschluss: Finaler Rechte-Quercheck

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/marketing-content/route.ts` verlangt jetzt fuer Lesen und Schreiben einen aktiven Actor.
- Marketing-Content-Kontingente (`saveQuota`) sind auf Admin und Geschaeftsfuehrung begrenzt.
- Operative Marketing-Content-Aktionen mit Planungswirkung (`generateItems`, `saveItem`, `saveSchedule`, `deleteSchedule`, `deleteItem`) sind auf Admin, Geschaeftsfuehrung und Fuehrungskraft begrenzt.
- `src/app/api/monthly-financial-report/route.ts` verlangt beim Schreiben manueller Monatsberichtswerte jetzt die bestehende Rechnungs-/Buchhaltungsberechtigung.
- Typische Alt-Risikomuster wie Demo-Admin-Fallbacks (`?? user`, `users[0]`) wurden im aktuellen API-/Lib-Stand erneut gesucht; fuer diese Muster gab es keine neuen Treffer.

Rollen:
- Marketing-Content lesen: aktive Benutzer.
- Marketing-Kontingente konfigurieren: Admin und Geschaeftsfuehrung.
- Marketing-Content operativ erzeugen/planen/loeschen: Admin, Geschaeftsfuehrung und Fuehrungskraft.
- Monatsberichtswerte bearbeiten: Admin, Geschaeftsfuehrung, Fuehrungskraft und Buchhaltung.

Bewusst nicht geaendert:
- Der Monatsbericht bleibt lesbar fuer aktive Benutzer, weil die UI-Auswertungsfreigaben nicht in diesem Block neu verengt wurden. Geaendert wurde gezielt der Schreibweg fuer manuelle Finanzwerte.
- Der Marketing-Content-Endpunkt wird im aktuellen Dashboard nicht sichtbar aufgerufen. Er wurde trotzdem serverseitig abgesichert, damit der Alt-/Restweg nicht offen bleibt.
- Eine echte Session-/Login-Haertung bleibt ein spaeterer Architekturblock. Phase 2 hat die request-basierten Actor-Wege konsequent gegen fehlende/ungueltige Actors und fehlende Rollenbremsen abgesichert.

Warum:
- Marketing-Content schreibt nicht nur eigene Datensaetze, sondern kann auch Planungseintraege erzeugen oder loeschen.
- Monatsberichtswerte sind Finanzdaten und duerfen nicht nur ueber eine aktive Benutzer-ID geschuetzt sein.
- Der Abschlusscheck sollte keine UI-Funktion erweitern, sondern verbleibende serverseitige Luecken schliessen.

Checks:
- `git diff --check -- src/app/api/marketing-content/route.ts src/app/api/monthly-financial-report/route.ts`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`

Phase-2-Status:
- Die priorisierten Rechte-/Actor-Luecken aus den aktiven Kernbereichen sind geschlossen.
- Verbleibende groessere Themen sind keine kleinen Rechte-Patches mehr, sondern Produkt-/Architekturentscheidungen: echte Session statt request-basierter Actor-ID, harte Loeschungen/Archivierungsmodell, Versionshistorien fuer Positionslisten und Runtime-DDL/Migrationen.

## Phase-2-Stabilisierung: Smoke-Test nach Rechtehaertung

Stand: 2026-06-21

Geprueft:
- Frischer Testserver auf Port 3002 gestartet, weil der alte Prozess auf Port 3001 noch aus einem frueheren Stand lief.
- `/` und `/dashboard` laden auf dem frischen Server mit HTTP 200.
- Ideenbereich, Content-Altbereich, Abrechnungswarnungen, Marketing-Content und Monatsfinanzbericht wurden mit und ohne aktiven Actor direkt gegen die lokale API geprueft.
- Erwartete Schutzantworten treten kontrolliert auf: fehlender Actor fuehrt zu 401, unberechtigte Rolle zu 403, fachlich ungueltige Eingabe bei berechtigtem Actor zu 400.

Ergebnis:
- Kein aktueller 500er im frischen Stand.
- Keine 401/403-Schleife im getesteten Dashboard-Load.
- Der 500er auf Port 3001 war ein Hinweis auf einen alten laufenden Serverprozess, nicht auf den aktuellen Codezustand.

Checks:
- `npm.cmd run check:regressions`
- Lokaler HTTP-Smoke-Test gegen `http://localhost:3002/dashboard`
- Lokale API-Smoke-Tests fuer `/api/idea-store`, `/api/content-items`, `/api/unbilled-time-alerts`, `/api/marketing-content` und `/api/monthly-financial-report`

Naechster sinnvoller Schritt:
- Mit dem naechsten groesseren Audit-Thema weitermachen: harte Loeschungen vs. Archivierung und Referenzschutz, beginnend bei Kontakten/Kundenakte.

## Phase-3-Start: Kontaktloeschung und Referenzschutz

Stand: 2026-06-21

Umgesetzt:
- Kontaktverwaltungsrechte in `src/lib/permissions/index.ts` zentralisiert.
- `src/app/api/contacts/route.ts` erlaubt Kontaktanlage und Kontaktbearbeitung nur noch fuer Admin, Geschaeftsfuehrung, Fuehrungskraft, Vertrieb und Buchhaltung.
- Endgueltiges Kontaktloeschen ist auf Admin und Geschaeftsfuehrung begrenzt.
- Vor dem endgueltigen Loeschen prueft die API jetzt vorhandene Bezuege in Projekten, Unterkontakten/Ansprechpartnern, aktiven Kundenhinweisen, Potenzialen, Verkaufschancen, Sales-Zielen, Feedback-Anfragen, Kundenfeedbacks und Winterdienstlaeufen.
- Wenn Bezuege bestehen, wird das Loeschen mit HTTP 409 blockiert und die Antwort nennt die betroffenen Bereiche mit Anzahl.
- `src/components/dashboard/dashboard-page.tsx` bricht Kontaktanlage/-bearbeitung/-loeschung ohne aktiven Benutzer direkt ab.

Rollen:
- Kontakte lesen: aktive Benutzer.
- Kontakte anlegen/bearbeiten/archivieren: Admin, Geschaeftsfuehrung, Fuehrungskraft, Vertrieb und Buchhaltung.
- Kontakte endgueltig loeschen: Admin und Geschaeftsfuehrung, aber nur ohne bestehende Bezuege.

Bewusst nicht geaendert:
- Die bestehende Archivierungslogik ueber Kategorie `Archiv` bleibt erhalten und ist weiterhin der sichere Weg fuer verknuepfte Kontakte.
- Historische Belege mit gespeicherten Kundennamen werden nicht nachtraeglich umgeschrieben.
- Ein vollstaendiges globales Archivierungsmodell fuer alle Tabellen bleibt ein eigener Architektur-/Produktblock.

Warum:
- Kontakte haengen an Kundenakte, Projektakte, Ansprechpartnern, Feedback, Vertrieb und Winterdienst.
- Hartes Loeschen ohne Referenzpruefung kann Projekt-/Kundenakten und Folgeprozesse inkonsistent machen.
- Blockieren mit konkreter Bezugsliste ist sicherer als stilles Loeschen oder automatisches Umhaengen.

Checks:
- `git diff --check -- src/app/api/contacts/route.ts src/lib/permissions/index.ts src/components/dashboard/dashboard-page.tsx`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test: 401 ohne Actor, 403 fuer Mitarbeiter-Schreiben/Loeschen, 200 fuer berechtigte Anlage, 409 fuer verknuepften Kontakt, 200 fuer unverketteten Testkontakt inklusive Cleanup.

Naechster sinnvoller Schritt:
- Den gleichen Referenzschutz-Ansatz auf Projektloeschung/Projektarchivierung und Logbuch-/Anhang-Bezuege uebertragen.

## Phase-3-Fortsetzung: Projektpflege, Archivierung und Kontaktbezug

Stand: 2026-06-21

Umgesetzt:
- Projektrechte in `src/lib/permissions/index.ts` zentralisiert.
- `src/app/api/hero/projects/route.ts` verlangt fuer Projektanlage und Projektbearbeitung jetzt einen aktiven Actor mit Projektverwaltungsrecht.
- Projektverwaltung ist auf Admin, Geschaeftsfuehrung, Fuehrungskraft und Vertrieb begrenzt.
- Projektarchivierung ueber Status `Archiviert` ist enger auf Admin, Geschaeftsfuehrung und Fuehrungskraft begrenzt.
- Kontakt-, Ansprechpartner- und Adresskontakt-IDs werden beim Projektspeichern gegen Kontakte derselben Organisation validiert.
- Ansprechpartner/Adresskontakt duerfen nicht offensichtlich zu einem anderen Hauptkontakt gehoeren.
- `src/components/dashboard/dashboard-page.tsx` bricht Projektspeichern ohne aktiven Benutzer sauber ab.

Rollen:
- Projekte lesen: aktive Benutzer.
- Projekte anlegen/bearbeiten: Admin, Geschaeftsfuehrung, Fuehrungskraft und Vertrieb.
- Projekte archivieren: Admin, Geschaeftsfuehrung und Fuehrungskraft.

Bewusst nicht geaendert:
- Es gibt weiterhin keinen harten Projekt-DELETE-Endpunkt. Projekte werden fachlich ueber Status/Archiv behandelt.
- Bestehende verknuepfte Belege, Zeiten, Aufgaben, Hinweise, Dokumente und Logbuchdaten bleiben beim Archivieren erhalten.
- Ein vollstaendiger Projekt-Loeschblock mit Referenzzaehlung ist aktuell nicht noetig, solange kein harter Projekt-DELETE existiert.

Warum:
- Der reale Risikopfad war nicht ein vorhandenes Projektloeschen, sondern das offene Projekt-Schreiben und Status-Archivieren.
- Falsche Kontakt-IDs koennen Kundenakte, Projektakte, Dokumentversand, Winterdienst und Rechnungsempfaengerlogik stoeren.
- Archivieren darf nicht versehentlich durch Rollen erfolgen, die nur operative Vertriebsdaten pflegen sollen.

Checks:
- `git diff --check -- src/app/api/hero/projects/route.ts src/lib/permissions/index.ts src/components/dashboard/dashboard-page.tsx`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test: 401 ohne Actor, 200 fuer Mitarbeiter-Lesen, 403 fuer Mitarbeiter-Schreiben, 201 fuer berechtigte Anlage, 400 fuer ungueltige Kontakt-ID, 201 fuer berechtigte Archivierung, Testprojekt danach entfernt.

Naechster sinnvoller Schritt:
- Logbuch-/Anhangwege pruefen: Upload, Verschieben und Loeschen von Projektbildern/Dokumentanhaengen gegen Rollen, Projektbezug und Archivstatus absichern.

## Phase-3-Fortsetzung: Projektlogbuch und Anhangschutz

Stand: 2026-06-21

Umgesetzt:
- Projektlogbuchrechte in `src/lib/permissions/index.ts` zentralisiert.
- `src/app/api/project-logbook-entries/route.ts` validiert beim Schreiben jetzt, ob das Projekt in derselben Organisation existiert.
- Normale Projektlogbuch-Eintraege und Uploads bleiben fuer aktive Nicht-Gast-Benutzer erlaubt, damit operative Mitarbeiter vor Ort weiter Bilder und Dokumente erfassen koennen.
- Anhang-Verschieben und Anhang-Loeschen ist nur noch fuer Projektverwaltungsrollen oder den urspruenglichen Autor des Logbucheintrags erlaubt.
- Archivierte Projekte sind fuer neue Logbucheintraege und Anhangveraenderungen gesperrt, ausser fuer Rollen mit Archiv-/Fuehrungsrecht.
- `src/components/dashboard/dashboard-page.tsx` zeigt Fehler aus automatischen Logbuch-Hilfsaufrufen jetzt an, statt sie still zu verschlucken.

Rollen:
- Projektlogbuch lesen: aktive Benutzer.
- Projektlogbuch erfassen/Uploads: aktive Benutzer ausser Gast.
- Projektanhaenge fremder Eintraege verschieben/loeschen: Admin, Geschaeftsfuehrung, Fuehrungskraft und Vertrieb.
- Eigene Anhaenge verschieben/loeschen: urspruenglicher Autor des Eintrags.
- Archivierte Projekte veraendern: nur Archiv-/Fuehrungsrollen.

Bewusst nicht geaendert:
- Mitarbeiter duerfen weiter operative Fotos und Dokumente hochladen. Eine zu enge Rollenbremse wuerde Baustellen-/Ausfuehrungsdokumentation brechen.
- Bestehende Data-URL-Speicherung wurde nicht auf Dateispeicher umgestellt. Das bleibt ein eigener Architekturblock.
- Historische Logbucheintraege werden nicht nachtraeglich veraendert.

Warum:
- Projektbilder, Taetigkeitsberichte und Dokumentanhaenge sind fachliche Nachweise und duerfen nicht beliebig entfernt oder verschoben werden.
- Gleichzeitig muss die mobile/operative Erfassung fuer Mitarbeiter erhalten bleiben.
- Archivierte Projekte sollen nachvollziehbar bleiben und nicht durch normale Folgeaktionen veraendert werden.

Checks:
- `git diff --check -- src/app/api/project-logbook-entries/route.ts src/lib/permissions/index.ts src/components/dashboard/dashboard-page.tsx`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test: 401 ohne Actor, 201 fuer Mitarbeiter-Upload, 403 fuer fremden Mitarbeiter beim Anhangloeschen, 200 fuer Fuehrungskraft beim Anhangloeschen, 403 fuer Mitarbeiter-Schreiben auf archiviertem Projekt, Testdaten danach entfernt.

Naechster sinnvoller Schritt:
- Den Architektur-/Datenblock fuer grosse Anhaenge vorbereiten: Data-URL-Speicherung, Groessenlimits, Speicherort und langfristige Performance bewerten.

## Phase-3-Fortsetzung: Anhanggroessen und Data-URL-Schutz

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/project-logbook-entries/route.ts` prueft Logbuchanhaenge jetzt serverseitig auf erlaubte Dateitypen und Data-URL-Format.
- Pro Anhang gilt ein hartes Limit von 12 MB.
- Pro Logbucheintrag gilt ein Gesamtlimit von 48 MB fuer alle Anhaenge zusammen.
- Die effektive Groesse wird aus gemeldeter Dateigroesse und Data-URL-Inhalt abgeschaetzt, damit direkte API-Aufrufe nicht durch kleine Fake-Size-Werte vorbeikommen.
- Erlaubt sind typische Bildformate sowie PDF, Word, Excel, CSV und TXT.
- `src/components/dashboard/dashboard-page.tsx` blockiert zu grosse oder unpassende Dateien bereits beim Auswaehlen bzw. vor dem Senden und zeigt eine einfache Fehlermeldung.
- Die separaten Upload-Wege fuer Projektbilder, Projektdokumente, manuelle Logbuchanhaenge und automatische Logbuch-Hilfsaufrufe verwenden dieselben Grenzen.

Bewusst nicht geaendert:
- Die bestehende Data-URL-Speicherung wurde noch nicht auf Datei-/Objektspeicher umgebaut.
- Historische Logbucheintraege und vorhandene grosse Anhaenge werden nicht migriert oder veraendert.
- Berichtsgeneratoren und Spezial-Endpunkte, die eigene Dateien erzeugen, bleiben ein eigener Folgecheck, falls dort direkte Data-URL-Speicherung ausserhalb des Logbuch-POSTs passiert.

Warum:
- Sehr grosse Base64/Data-URL-Anhaenge koennen Datenbank, Browser und API-Antworten stark belasten.
- Eine Groessenbremse ist ein kurzfristiger Sicherheitsgewinn, ohne die spaetere Speicherarchitektur vorwegzunehmen.
- Operative Foto- und Dokumentationserfassung bleibt erhalten, bekommt aber klare Grenzen.
...
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test: 413 fuer zu grossen PDF-Anhang, 400 fuer unerlaubten EXE-Typ, 201 fuer kleine erlaubte TXT-Datei; Testeintrag danach entfernt.

Naechster sinnvoller Schritt:
- Spezial-Endpunkte fuer erzeugte Berichte und sonstige direkte Anhang-Erzeugung gegen dieselben Groessen-/Typgrenzen pruefen.

## Phase-3-Fortsetzung: Berichtsendpunkte und direkte PDF-Anhaenge

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/activity-reports/route.ts` nutzt jetzt dieselben Grundregeln wie das Projektlogbuch: aktiver Actor, Logbuch-Schreibrecht und Archivschutz.
- Taetigkeitsberichte werden beim direkten PDF-Schreiben ins Projektlogbuch auf 12 MB begrenzt.
- `src/app/api/smoke-detector-reports/route.ts` nutzt ebenfalls aktiven Actor, Logbuch-Schreibrecht und Archivschutz.
- Rauchmelderbilder werden vor der PDF-Erstellung auf erlaubte Bildtypen, gueltige Data-URLs, 12 MB pro Bild und 48 MB pro Nachweis begrenzt.
- Der erzeugte Rauchmelder-PDF-Anhang wird vor dem Speichern auf 12 MB begrenzt.
- Beide Berichtsendpunkte schreiben nicht mehr anonym als `System`, sondern mit dem aktiven Benutzer als Autor.

Bewusst nicht geaendert:
- Die PDF-Generierung selbst wurde fachlich nicht umgebaut.
- Historische Berichte und bestehende grosse PDF-Anhaenge bleiben unveraendert.
- Ein gemeinsames Attachment-Policy-Modul wurde noch nicht ausgelagert; wegen Audit-Sicherheit wurden die Aenderungen gezielt in den betroffenen Endpunkten gehalten.

Warum:
- Diese Endpunkte konnten PDFs direkt ins Logbuch schreiben und mussten deshalb dieselben Schutzlinien bekommen wie normale Logbuch-Uploads.
- Rauchmelderbilder werden direkt an den Berichtsendpunkt geschickt und muessen vor schwerer PDF-Verarbeitung begrenzt werden.
- Archivierte Projekte sollen auch ueber automatische Berichtserzeugung nicht veraendert werden.

Checks:
- `git diff --check -- src/app/api/activity-reports/route.ts src/app/api/smoke-detector-reports/route.ts`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test: 401 ohne Actor fuer beide Endpunkte, 413 fuer zu grosses Rauchmelderbild, 403 fuer normale Rolle auf archiviertem Projekt bei Taetigkeits- und Rauchmelderbericht; Testprojekt danach entfernt. Gast-Test wurde uebersprungen, weil in den lokalen Daten kein aktiver Gast vorhanden war.

Naechster sinnvoller Schritt:
- Abschluss-Quercheck fuer verbleibende direkte `ProjectLogbookEntry`-Schreibstellen und sonstige direkte Data-URL-Speicherpfade.

## Phase-3-Fortsetzung: Abschluss-Quercheck direkte Logbuch-/Data-URL-Pfade

Stand: 2026-06-21

Umgesetzt:
- Direkte `ProjectLogbookEntry`-Schreibstellen und Data-URL-Speicherpfade wurden erneut gesucht.
- `src/app/api/winter-service-runs/route.ts` wurde als echter Restpfad abgesichert, weil dort Winterdienstbilder, Winterdienst-PDFs und Projektlogbuch-Eintraege direkt verarbeitet werden.
- Winterdienst-Einsaetze verlangen jetzt aktiven Actor und Logbuch-Schreibrecht.
- Das Zielprojekt muss zur Organisation gehoeren; archivierte Projekte sind fuer normale Rollen gesperrt.
- Winterdienstbilder werden auf gueltige Data-URLs, erlaubte Bildtypen, 12 MB pro Bild und 48 MB pro Einsatz begrenzt.
- Erzeugte Winterdienst-Taetigkeitsberichte werden vor dem Speichern auf 12 MB begrenzt.
- Der direkte Logbuch-PDF-Eintrag schreibt den aktiven Benutzer als Autor statt anonym `System`.

Bewusst nicht geaendert:
- Winterdienst-Automatik schreibt weiterhin kleine System-Logbucheintraege ohne Anhaenge. Das ist kein Data-URL-Speicherpfad.
- Endkontrolle erzeugt nur einen kleinen Textanhang aus Servertext. Kein Benutzer-Dateiupload, kein grosses Binarrisiko.
- Rechnungs-/Mahnungs-PDFs, Angebote und andere fachliche PDF-Speicherungen bleiben eigene Produkt-/Speicherarchitekturthemen; sie sind keine offenen Uploadpfade.
- Dokument-Mail-Anhaenge werden versendet, aber nicht als neuer Data-URL-Speicherpfad persistiert.

Warum:
- Der relevante Restpfad war Winterdienst: dort konnten Bilder direkt in eine Tabelle und erzeugte PDFs direkt ins Logbuch laufen.
- Die neuen Grenzen verhindern grosse Base64-Daten vor Datenbank- und PDF-Verarbeitung.
- Archivschutz und Autorbezug bleiben konsistent mit den vorherigen Projektlogbuch-Regeln.

Checks:
- `git diff --check -- src/app/api/winter-service-runs/route.ts`
- `npm.cmd run build`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test: 401 ohne Actor, 413 fuer zu grosses Winterdienstbild, 201 fuer kleinen Testeinsatz mit Cleanup, 403 fuer normale Rolle auf archiviertem Projekt.

Naechster sinnvoller Schritt:
- Phase-3 Abschlusslauf: dokumentierte Restentscheidungen konsolidieren, noch offene Architekturthemen sammeln und eine finale Risiko-/Prioritaetenliste erstellen.

## Phase-3-Abschluss: Referenzen, Archivschutz und Anhangpfade

Stand: 2026-06-21

Status:
- Phase 3 ist fuer die priorisierten Schutz- und Crash-Risiken abgeschlossen.
- Komplettes Audit nach aktuellem Stand: ca. 76-78%.
- Kritische Rechte-, Absturz- und Datenverlustschutz-Themen: ca. 90-92%.

Abgesichert:
- Kontaktloeschung ist auf Admin/Geschaeftsfuehrung begrenzt und blockiert verknuepfte Kontakte mit HTTP 409.
- Projektanlage/-bearbeitung verlangt aktive berechtigte Benutzer; Projektarchivierung ist enger begrenzt.
- Projekt-Kontaktbezug wird gegen Kontakte derselben Organisation validiert.
- Projektlogbuch-Schreibwege validieren Projekt/Organisation und Archivstatus.
- Projektanhaenge fremder Eintraege koennen nur noch durch Projektverwaltungsrollen oder den urspruenglichen Autor verschoben/geloescht werden.
- Logbuchanhaenge haben Typ- und Groessenlimits: 12 MB pro Datei, 48 MB pro Eintrag.
- Taetigkeitsberichte, Rauchmelderberichte und Winterdienstberichte respektieren Actor, Logbuch-Schreibrecht, Archivschutz und Groessenlimits.
- Direkte Logbuch-/Data-URL-Schreibpfade wurden erneut gesucht; der relevante Restpfad Winterdienst wurde abgesichert.

Bewusst offen als Architektur-/Produktentscheidungen:
- Echte Session-/Server-Auth statt request-basierter `actorId`.
- Datei-/Objektspeicher statt langfristiger Base64/Data-URL-Speicherung in JSON.
- Historische grosse Anhaenge werden nicht automatisch migriert oder veraendert.
- Globales Archivierungsmodell fuer alle Tabellen statt einzelner Fachregeln.
- Vollstaendige harte Projektloeschung mit Referenzzaehlung ist aktuell nicht umgesetzt, weil es keinen harten Projekt-DELETE-Endpunkt gibt.
- Rechnungs-, Angebots- und Mahnungs-PDFs bleiben fachliche PDF-Speicherpfade und sollten spaeter im Speicherarchitekturblock betrachtet werden.
- Runtime-DDL in API-Routen bleibt ein spaeterer Migrations-/Deployment-Architekturblock.

Rest-Risiko nach Phase 3:
- Niedrig bis mittel bei operativen Projekt-/Logbuchaktionen, weil direkte Schutzluecken geschlossen und per Smoke-Test geprueft wurden.
- Mittel bei langfristiger Performance/Speicherlast, solange grosse historische Data-URLs in der Datenbank bleiben koennen.
- Mittel bei Auth-Sicherheit, solange der Demo-/ActorId-Ansatz nicht durch echte Sessions ersetzt ist.
- Niedrig bei versehentlichem Datenverlust durch Kontakt-/Projektaktionen im geprueften Bereich, weil Loesch- und Archivschutz jetzt greifen.

Empfohlene naechste Prioritaeten:
1. Stabilitaets-/Performanceblock fuer grosse Datenmengen, Ladepfade und Data-URL-Folgen.
2. Architekturblock: echte Auth/Session, zentrale Attachment-Policy und Datei-/Objektspeicher.
3. Migrationsblock: Runtime-DDL aus API-Routen in saubere Migrationen ueberfuehren.
4. Produktentscheidung: globales Archivierungs-/Loeschkonzept fuer Projekte, Dokumente und Finanzdaten.
5. Abschluss-Smoke: einmal kompletter UI-Fluss fuer Projektakte, Logbuch, Berichtserstellung, Dokumentversand und Archivprojekt.

Checks:
- Phase-3-Einzelbloecke wurden jeweils mit `npm.cmd run build`, `npx.cmd prisma validate`, `npm.cmd run check:mojibake`, `npm.cmd run check:regressions` und gezielten lokalen API-Smoke-Tests geprueft.
- Abschlusslauf hat die dokumentierten Restentscheidungen konsolidiert und die verbleibenden Themen als Architektur-/Produktentscheidungen getrennt.

Naechster sinnvoller Schritt:
- Mit dem Stabilitaets-/Performanceblock beginnen: Ladepfade, grosse Datenmengen und Abschluss-Smoke der wichtigsten UI- und API-Flows.

## Stabilitaets-/Performanceblock: leichte Logbuch-Startladung

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/project-logbook-entries/route.ts` liefert bei `summary=1` jetzt auch fuer die globale Logbuchabfrage eine leichte Antwort ohne `dataUrl`.
- Die Summary enthaelt weiter Projekt-ID, Datum, Titel, Text, Autor, Sichtbarkeit, Monat, Aktualisierungszeit und Anhang-Metadaten.
- `src/components/dashboard/dashboard-page.tsx` nutzt beim Dashboard-Start jetzt `summary=1`.
- Projektbezogene Detailabfragen bleiben unveraendert vollstaendig und laden Anhang-Data-URLs erst, wenn ein konkretes Projekt geoeffnet oder synchronisiert wird.

Messung:
- Lokaler API-Smoke-Test mit 257 Logbucheintraegen:
  - volle globale Antwort: ca. 32.071.362 Bytes
  - neue Summary-Antwort: ca. 116.877 Bytes
  - Summary enthaelt keine `dataUrl`-Keys
- Das reduziert den schweren Startpfad massiv, ohne Projektakten-Detailansichten zu entfernen.

Bewusst nicht geaendert:
- Die Detailansicht eines Projekts laedt weiterhin volle Anhaenge, weil Bilder/PDFs dort geoeffnet und angezeigt werden muessen.
- Die langfristige Umstellung auf Datei-/Objektspeicher bleibt ein Architekturblock.
- Weitere grosse Startdaten wie Angebote, Rechnungen, Kontakte und Projekte wurden in diesem Schritt noch nicht paginiert.

Warum:
- Der globale Dashboard-Start hat bisher alle Projektlogbuch-Data-URLs geladen.
- Bei vielen Fotos/PDFs fuehrt das zu grossen Antwortgroessen, langen Ladezeiten und hoher Browser-Speicherlast.
- Die vorhandene projektbezogene Nachlade-Logik erlaubt eine risikoarme Entkopplung: global leicht, projektbezogen voll.
...
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test: globale Summary 200 ohne `dataUrl`, volle globale Antwort weiterhin 200 mit `dataUrl` fuer Detail-/Kompatibilitaetsfaelle.

Naechster sinnvoller Schritt:
- Weitere Startladepfade bewerten: Angebote/Rechnungen/Kontakte/Projekte und Dokument-Mail-Historie auf unnoetig grosse Payloads, fehlende Limits oder zu breite Initialloads pruefen.

## Stabilitaets-/Performanceblock: Dokument-Mail-Uebersicht begrenzt

Stand: 2026-06-21

Umgesetzt:
- `src/components/dashboard/dashboard-page.tsx` ruft die globale Dokument-Mail-Historie jetzt mit `limit=500` ab.
- `src/app/api/document-mail/route.ts` erzwingt fuer globale Dokument-Mail-Abfragen ein begrenztes Ergebnis: Standard 500, maximal 1000 Eintraege.
- Projektbezogene Dokument-Mail-Abfragen bleiben unveraendert vollstaendig, damit die Projektakte ihre Historie weiter komplett anzeigen kann.

Warum:
- Die globale Dokument-Mail-Historie wird fuer Uebersichten und Statuslogik genutzt, aber darf beim Dashboard-Start nicht unbegrenzt wachsen.
- Detailhistorien werden weiterhin erst pro Projekt geladen, wo der Benutzer die vollstaendige Historie erwartet.

Bewusst nicht geaendert:
- Die globale Historie wurde nicht komplett entfernt, weil Versandstatus- und Automatikuebersichten diese Daten verwenden.
- Keine Pagination im UI; dieser Schritt ist ein Startschutz, kein vollstaendiger Historienbrowser.
...
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- Lokaler API-Smoke-Test auf Port 3002: `/api/document-mail?actorId=...&limit=2` antwortete mit HTTP 200. Die lokale Tabelle war leer; deshalb konnte keine projektbezogene Folgeantwort mit Daten geprueft werden.

Naechster sinnvoller Schritt:
- Weiterer Startladepfad: Angebote/Rechnungen/Projektzeiten darauf pruefen, ob globale Startlisten fuer die aktuellen Ansichten wirklich voll geladen werden muessen oder ob leichte Zusammenfassungen/Filter moeglich sind.

## Stabilitaets-/Performanceblock: Angebots- und Rechnungslisten ohne PDF-Ballast

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/offers/route.ts` nutzt fuer Angebotslisten jetzt konkrete Spalten statt `SELECT *`.
- `src/app/api/invoices/route.ts` nutzt fuer Rechnungslisten jetzt konkrete Spalten statt `SELECT *`.
- Gespeicherte PDF-Base64-Daten werden in Listenabfragen nicht mehr aus der Datenbank mitgelesen.
- `pdfAvailable` bleibt erhalten, weil die Listen nur einen Platzhalter fuer vorhandene PDFs erhalten.
- Der separate PDF-Abruf per `pdfId` bleibt unveraendert und liefert weiterhin das echte PDF erst bei Bedarf.

Warum:
- Dashboard- und Listenansichten brauchen nicht den PDF-Inhalt jeder Rechnung/jedes Angebots.
- Bisher wurde `pdfData` intern durch `SELECT *` trotzdem mitgeladen, obwohl die API-Antwort es spaeter entfernt hat.
- Die Aenderung reduziert Datenbank-/Serverlast, ohne UI-Funktionen oder Dokumentabruf zu entfernen.

Checks:
- `git diff --check -- src/app/api/offers/route.ts src/app/api/invoices/route.ts`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- `npm.cmd run build`
- Lokaler API-Smoke-Test auf Port 3002:
  - `/api/offers` HTTP 200, 3 Eintraege, keine `pdfData`-Nutzdaten in der Liste
  - `/api/invoices` HTTP 200, 4 Eintraege, keine `pdfData`-Nutzdaten in der Liste

Naechster sinnvoller Schritt:
- Projektzeiten-GET absichern und bewerten: aktuell ist der Startladepfad breit und sollte auf Actor/saubere Filter oder spaetere Zusammenfassung geprueft werden.

## Stabilitaets-/Rechteblock: Projektzeiten-Lesen mit Actor

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/project-time-entries/route.ts` verlangt beim Lesen jetzt `actorUserId`.
- Ohne aktiven Benutzer antwortet die API mit HTTP 401.
- Rollen mit Projektzeit-Verwaltungsrecht sehen weiterhin alle Projektzeiten.
- Normale Benutzer sehen beim Lesen nur eigene Projektzeiten.
- `src/components/dashboard/dashboard-page.tsx` sendet beim Laden der Projektzeiten jetzt die aktive Benutzer-ID mit.
- Beim spaeteren Setzen/Wechseln des aktiven Benutzers werden Projektzeiten erneut geladen.

Warum:
- Der bisherige GET-Pfad lieferte alle Projektzeiten ohne Actor-Pruefung.
- Projektzeiten enthalten abrechnungs- und personenbezogene Daten und sollten nicht global ohne aktiven Benutzer lesbar sein.
- Die Mitarbeiter-Emulation bleibt erhalten: Wenn ein Mitarbeiter als aktiver Benutzer laeuft, sieht er die Mitarbeiter-Sicht; Managerrollen behalten die breite Verwaltungs-/Abrechnungssicht.

Bewusst nicht geaendert:
- Das Antwortformat bleibt gleich, damit Abrechnung, Projektakte, Mitarbeiteransichten und Stempel-Auswertungen nicht umgebaut werden muessen.
- Noch keine Pagination/Summary fuer Projektzeiten; der lokale Smoke zeigt aktuell nur 35 Eintraege und ca. 33 KB Antwortgroesse.
...
- `npm.cmd run build`
- Lokaler API-Smoke-Test auf Port 3002:
  - `/api/project-time-entries` ohne Actor: HTTP 401
  - `/api/project-time-entries?actorUserId=...`: HTTP 200, 35 Eintraege, ca. 33 KB

Naechster sinnvoller Schritt:
- Uebrige breite Startpfade kurz querpruefen und dann einen Abschluss-Smoke fuer die wichtigsten UI-Flows vorbereiten.

## Stabilitaets-/Rechteblock: Angebote/Rechnungen lesen mit Actor

Stand: 2026-06-21

Umgesetzt:
- `src/app/api/offers/route.ts` verlangt beim Lesen jetzt `actorId`.
- `src/app/api/invoices/route.ts` verlangt beim Lesen jetzt `actorId`.
- Ohne aktiven Benutzer antworten beide APIs mit HTTP 401.
- Lesen ist auf Arbeitsrollen mit Dokument-/Mail-Kontext begrenzt: Admin, Geschaeftsfuehrung, Fuehrungskraft, Vertrieb und Buchhaltung.
- `src/components/dashboard/dashboard-page.tsx` sendet bei Angebots-/Rechnungslisten, Historien, PDF-Links und XRechnung-Links die aktive Benutzer-ID mit.
- Beim aktiven Benutzerwechsel werden Angebote und Rechnungen erneut geladen.

Warum:
- Angebots- und Rechnungsdaten enthalten Geschaefts- und Finanzinformationen und sollten nicht ohne aktiven Benutzer global lesbar sein.
- Der vorherige Performanceblock hatte PDF-Ballast aus den Listen entfernt; dieser Block schliesst die passende Leseschutz-Luecke.
- PDF- und XRechnung-Abrufe bleiben funktional, laufen aber ebenfalls ueber den Actor-geschuetzten GET-Pfad.

Checks:
- `git diff --check -- src/app/api/offers/route.ts src/app/api/invoices/route.ts src/components/dashboard/dashboard-page.tsx`
- `npx.cmd prisma validate`
- `npm.cmd run check:mojibake`
- `npm.cmd run check:regressions`
- `npm.cmd run build`
- Lokaler API-Smoke-Test auf Port 3002:
  - `/api/offers` ohne Actor: HTTP 401
  - `/api/invoices` ohne Actor: HTTP 401
  - `/api/offers?actorId=...`: HTTP 200, 3 Eintraege
  - `/api/invoices?actorId=...`: HTTP 200, 4 Eintraege

Naechster sinnvoller Schritt:
- Abschluss-Smoke vorbereiten: Dashboard-Start, Projektakte, Logbuch/Anhaenge, Angebote/Rechnungen/PDF, Projektzeiten, Dokument-Mail und Winterdienst-/Berichtspfade einmal zusammen pruefen.

## Abschluss-Smoke: Kernpfade nach Stabilitaets-/Rechtebloecken

Stand: 2026-06-21

Geprueft auf lokalem Testserver Port 3002:
- `/dashboard`: HTTP 200
- `/api/hero/projects?actorId=...`: HTTP 200, 132 Projekte, ca. 148 KB
- `/api/project-logbook-entries?actorId=...&summary=1`: HTTP 200, 257 Eintraege, ca. 117 KB, weiterhin leichte Summary
- `/api/project-time-entries?actorUserId=...`: HTTP 200, 35 Eintraege, ca. 33 KB
- `/api/offers?actorId=...`: HTTP 200, 3 Eintraege, ca. 5 KB
- `/api/invoices?actorId=...`: HTTP 200, 4 Eintraege, ca. 8 KB
- `/api/document-mail?actorId=...&limit=10`: HTTP 200, lokale Historie leer
- `/api/contacts?actorId=...`: HTTP 200, 291 Kontakte, ca. 291 KB
- `/api/customer-feedback?actorId=...`: HTTP 200
- `/api/customer-feedback-requests?actorId=...`: HTTP 200
- Angebots-PDF mit Actor: HTTP 200, `application/pdf`
- Rechnungs-PDF mit Actor: HTTP 200, `application/pdf`
- XRechnung-Validierung mit Actor: HTTP 200

Schutzpruefungen:
- Angebote ohne Actor: HTTP 401
- Rechnungen ohne Actor: HTTP 401
- Projektzeiten ohne Actor: HTTP 401
- Dokument-Mail ohne Actor: HTTP 401
- Logbuch-Summary ohne Actor: HTTP 401
- Kontakte ohne Actor: HTTP 401
- Winterdienst GET ohne Actor: HTTP 401
- Rauchmelderbericht POST ohne Actor: HTTP 401
- Winterdienst POST ohne Actor: HTTP 401
- Dokument-Mail POST ohne Actor: HTTP 401
- Taetigkeitsbericht POST mit plausibler Form, aber ohne Actor: HTTP 401
- Projektzeit POST mit plausibler Form, aber ohne Actor: HTTP 401

Einordnung:
- Im Smoke-Test traten keine HTTP-500-Antworten auf.
- Leere POST-Bodies bei Taetigkeitsbericht und Projektzeiten antworten zuerst mit HTTP 400, weil Pflichtdaten fehlen; mit plausibler Form ohne Actor greift jeweils HTTP 401.
- Groesster verbleibender Startdatenblock ist aktuell `contacts` mit ca. 291 KB. Das ist kein akuter Fehler, aber ein sinnvoller spaeterer Performancepunkt, falls Kontakte deutlich wachsen.

Naechster sinnvoller Schritt:
- Auditabschluss vorbereiten: Restthemen als Architektur-/Produktentscheidungen konsolidieren und nur noch echte Blocker vor Abschluss anfassen.

## Auditabschluss-Vorbereitung: konsolidierter Stand

Stand: 2026-06-21

Status:
- Das operative Audit ist nach Phase 1 bis 3 sowie den anschliessenden Stabilitaets-/Performancebloecken weitgehend abgeschlossen.
- Komplettes Audit nach aktuellem Stand: ca. 88-90%.
- Kritische Rechte-, Crash-, Datenverlust- und schwere Startlast-Themen: ca. 94-95%.
- Im Abschluss-Smoke wurden keine HTTP-500-Fehler in den geprueften Kernpfaden gefunden.

Erledigte Kernschutzbereiche:
- Actor-Pflicht wurde fuer zentrale interne Lese- und Schreibpfade nachgezogen.
- Kontaktloeschung ist referenzgeschuetzt und rollenbegrenzt.
- Projektanlage, Projektpflege und Archivierung sind rollen- und organisationsgebunden.
- Projektlogbuch, Anhaenge, Taetigkeitsberichte, Rauchmelderberichte und Winterdienstpfade pruefen Actor, Projektbezug, Archivschutz und Anhanggroessen.
- Angebote, Rechnungen, Projektzeiten, Dokument-Mail, Kontakte, Kundenfeedback, Abwesenheiten, Planung, Content-/Ideenpfade und weitere Phase-2-Bereiche sind fuer die priorisierten Actor-/Rechterisiken abgesichert.
- Grosse globale Logbuch-Startladung wurde von voller Anhangsladung auf leichte Summary umgestellt.
- Dokument-Mail-Uebersicht wurde begrenzt.
- Angebots-/Rechnungslisten lesen keine gespeicherten PDF-Base64-Daten mehr mit.
- Kernpfade wurden per lokalem Smoke-Test auf Port 3002 zusammen geprueft.

Bewusst offen, nicht als akuter Blocker:
- Echte Session-/Server-Auth statt request-basierter `actorId`/`actorUserId`.
- Datei-/Objektspeicher fuer Anhaenge und PDFs statt langfristiger Base64-/Data-URL-Speicherung.
- Saubere Migrationen statt Runtime-DDL in API-Routen.
- Globales Archivierungs-/Loeschmodell ueber alle fachlichen Tabellen.
- Kontakt- und Projektlisten koennen spaeter weiter paginiert oder in leichte Start-Summaries getrennt werden.
- Historische grosse Anhaenge/PDFs wurden nicht migriert.
- Oeffentlicher Kundenfeedback-Tokenpfad bleibt bewusst ohne internen Actor.

Rest-Risiko nach aktuellem Stand:
- Niedrig bei den geprueften operativen Klick-/Ladepfaden, weil Abschluss-Smoke keine 500er gezeigt hat.
- Niedrig bis mittel bei Berechtigungen im priorisierten Umfang, weil die wichtigsten internen Actor-Luecken geschlossen sind.
- Mittel bei Architektur/Auth, solange noch kein echtes Sessionmodell existiert.
- Mittel bei langfristiger Speicher-/Performance-Entwicklung, solange grosse Dateien in Datenbankfeldern liegen koennen.
- Niedrig bis mittel bei Altbereichen, die bewusst nicht mehr stark im UI genutzt werden; sie sollten nicht ohne eigenen Produktentscheid breit umgebaut werden.

Empfohlene naechste Schritte nach Auditabschluss:
1. Keine neuen breiten Feature-Umbauten direkt im Auditabschluss starten.
2. Nur noch echte Blocker beheben, falls sie im manuellen Durchklicken auffallen.
3. Danach eigener Architekturblock: Session/Auth, Datei-/Objektspeicher, Migrationen.
4. Danach eigener Performanceblock: Kontakte/Projekte paginieren oder Start-Summaries einfuehren, falls Datenmengen weiter wachsen.
5. Danach Produktentscheidung: globales Archivierungs-/Loeschkonzept fuer Projekte, Dokumente und Finanzdaten.

Aktuelle Abschlussbewertung:
- Das System ist fuer die geprueften Kernfluesse deutlich stabiler und besser abgesichert als zu Beginn des Audits.
- Es gibt nach dem Smoke-Test keinen bekannten akuten Absturzblocker in Dashboard, Projektakte, Logbuch-Summary, Projektzeiten, Angeboten/Rechnungen/PDF, Dokument-Mail, Kundenfeedback oder den geprueften Berichtspfaden.
- Die verbleibenden Punkte sind ueberwiegend Architektur-/Produktentscheidungen und sollten nicht als kleine Nebenfixes vermischt werden.

## Operativer Auditabschluss

Stand: 2026-06-21

Status:
- Das Audit wird fuer den aktuell priorisierten operativen Umfang als abgeschlossen markiert.
- Komplettes Audit: ca. 90%.
- Kritische Rechte-, Crash-, Datenverlust- und schwere Startlastthemen: ca. 95%.
- Keine bekannten akuten 500er-Blocker nach Abschluss-Smoke und finalem Build.

Finale Checks:
- `npx.cmd prisma validate`: bestanden.
- `npm.cmd run check:mojibake`: bestanden.
- `npm.cmd run check:regressions`: bestanden.
- `git diff --check` fuer die geaenderten Auditdateien und Kerncodepfade: bestanden.
- `npm.cmd run build`: bestanden.

Abschlussregel:
- Ab jetzt sollten keine weiteren breit wirkenden Codeaenderungen mehr unter dem laufenden Auditabschluss gemacht werden.
- Neue Arbeiten sollen als eigene Folgeblocks geplant werden:
  1. Architekturblock Auth/Session.
  2. Architekturblock Datei-/Objektspeicher.
  3. Migrationsblock Runtime-DDL.
  4. Performanceblock Kontakte/Projekte.
  5. Produktblock Archivierungs-/Loeschkonzept.
