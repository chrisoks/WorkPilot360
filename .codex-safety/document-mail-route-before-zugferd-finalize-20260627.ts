...
import { validateZugferdPdfA3 } from "@/lib/e-invoice/zugferd-validator";
...
async function getZugferdAttachment(organizationId: string, invoiceId: string, documentNumber: string) {
  ...
  const xml = Buffer.from(String(xrechnungAttachment.contentBytes || ""), "base64");
  await pdfDoc.attach(xml, "factur-x.xml", {
  ...
  const zugferdValidation = await validateZugferdPdfA3(Buffer.from(bytes));
  if (!zugferdValidation.available) {
    throw new Error("ZUGFeRD konnte nicht erzeugt werden: PDF/A-3-Validator ist nicht konfiguriert.");
  }
  if (!zugferdValidation.valid) {
    const issueText = zugferdValidation.issues.map((issue) => issue.message).filter(Boolean).slice(0, 3).join(" ");
    throw new Error(
      `ZUGFeRD wurde vom PDF/A-3-Validator abgelehnt: ${issueText || zugferdValidation.message}`
    );
  }
