...
function formatMonthFileLabel(value: string) {
  return formatMonthLabel(value).replace(/\s+/g, "_");
}
...
function buildActivityReportName(project: ProjectRow, month: string, useMonth: boolean) {
  const fallbackMonth = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, "0")}`;
  const monthLabel = formatMonthFileLabel(useMonth ? month : fallbackMonth);
  const projectNumber = sanitizeFilePart(project.projectNumber || project.id || "Projekt");
  return `Tätigkeitsbericht_${projectNumber}_${monthLabel}`;
}
...
function getReportImages(entries: ProjectLogbookEntryRow[], category: "Vorherbilder" | "Nachherbilder", month: string, useMonth: boolean) {
  return entries
    ...
    .filter((entry) => !useMonth || getMonthKey(entry.createdAt) === month)
    .flatMap((entry) =>
      cleanAttachments(entry.attachments)
        .filter((attachment) => attachment.type === "Bild" && attachment.dataUrl)
        .map((attachment) => ({ ...attachment, entryDate: entry.createdAt }))
    );
}
...
async function generateActivityReportPdf(input: {
  ...
  const infoRows = [
    ["Tätigkeitsbericht", input.reportName],
  ...
  drawText(firstPage, "Tätigkeitsbericht", 71, 520, fonts.bold, 10.7, INK);
  ...
  drawText(firstPage, salutation, 71, 492, fonts.regular, 8.8);
  drawWrappedText(
    firstPage,
  ...
  if (input.useMonth) {
    drawText(firstPage, "Berichtsmonat", 71, 346, fonts.bold, 8.8, MUTED);
    drawText(firstPage, formatMonthLabel(input.month), 205, 346, fonts.regular, 8.8);
  }
...
export async function POST(req: Request) {
  ...
  const reportName = buildActivityReportName(project, month, useMonth);
