...
- JARVIS Zeiteintragsverwaltung 2026-08-02: Bestehende Zeiten dürfen nur über den gemeinsamen Fachservice `src/lib/time/project-time-entry-management-service.ts` korrigiert oder logisch gelöscht werden; direkte Browser-Historien oder unkontrollierte Upserts sind verboten. Rechnungsgebundene und bereits gelöschte Einträge sperren fail-closed. Projekt, Mitarbeiter, Herkunft, Planung, Marketingbezug und historischer Stundenkostensatz bleiben geschützt. Korrekturen/Löschungen benötigen einen Grund, vollständigen Zeilenfingerprint, serialisierbare Transaktion, Advisory-Lock, Zeilensperre und serverseitige Bearbeitungshistorie. JARVIS `time.manage` ist zusätzlich organisations-, sitzungs-, rollen-, identitäts-, revisions-, HMAC- und kontextgebunden und verlangt exakt `ZEITEINTRAG KORRIGIEREN <ID>` oder `ZEITEINTRAG LÖSCHEN <ID>`. Der permanente Korpus bleibt exakt 110; isolierte QA liegt in `scripts/qa-jarvis-time-entry-management.mjs`. Keine Prisma-Schemaänderung und keine Änderung an `StoredFile`, privatem S3-Speicher oder Online-Anfragen-Invarianten.
- JARVIS Terminserienverwaltung 2026-08-02: `planning.request.manage` darf eine vollständige gespeicherte Serie nur über `cancel_series` oder `withdraw_series` verändern. Pflicht sind die vollständige ID eines sichtbaren Serieneintrags, Grund und exakt `TERMIN-SERIE ABSAGEN <ID>` beziehungsweise `TERMINWUNSCH-SERIE ZURÜCKZIEHEN <ID>`. Vorschau und Fingerprint binden alle aktiven Eintrag-IDs, Anzahl und Zeitraum; gemischte Freigabestatus sperren. Mitarbeiter dürfen nur eine vollständig eigene offene Wunschserie ohne Vertretung/Impersonation zurückziehen, Serienabsagen bleiben Planungsverantwortlichen vorbehalten. Normale Oberfläche und JARVIS verwenden `src/lib/planning/planning-request-decision-service.ts`; die serialisierbare Serienausführung nutzt Advisory-/Zeilensperren und schreibt Soft-Deletes, je Eintrag Historie, Projektlogbuch und Hinweise atomar und exactly-once. Einzelentscheidungen bleiben einzeln, Altwege bleiben gesperrt. Produktivabnahme Runtime `1ee7e01d112396e2b98944fc6d2228139ed05e78`, Backup `/var/backups/workpilot360/20260802T163736Z-before-jarvis-planning-series`, 187/187 Testdateien, 1.851/1.851 Tests, 90-Seiten-Build, lokale/produktive QA, Zwei-Termine-Klicktest, 110/110, null Rückstände, leerer Prisma-Diff, WorkPilot PID `788731`, KlinikNavigator unverändert `398228`. Keine Prisma-Schemaänderung; `StoredFile`, privater S3-Speicher und Online-Anfragen-Invarianten bleiben erhalten.
- JARVIS geführte Angebots- und Nachtragserstellung 2026-08-02: Natürliche Schreibwünsche starten einen geführten Dialog aus Kundenvolltextsuche, ausschließlich offenen Kundenprojekten, Angebotsgrunddaten, Katalogvolltextsuche und geprüfter Kalkulation. Die gemeinsame Suche ist organisations-, sitzungs- und rollenbegrenzt und tokenisiert Bindestriche sowie Wortabstände tolerant; große Projekt- und Kataloglisten dürfen im geführten Pfad nicht wieder vollständig ausgeliefert werden. Projektwahl und weitere Änderungen bleiben im persistenten revisions-/HMAC-/fingerprintgebundenen `offer.prepare`-Entwurf; nur die ausdrückliche Schlussbestätigung darf über `src/lib/offers/offer-draft-service.ts` genau einen Entwurf anlegen. Suche und Klickführung verursachen keine zusätzlichen OpenAI-Aufrufe. Nachträge verwenden denselben Weg mit weiterhin verpflichtender Nachtragsart und gültigem Bezugsangebot. Produktivabnahme Runtime `aa617f7704bd2bcc779af86df825275e4836773d`, Backup `/var/backups/workpilot360/20260802T194900Z-before-jarvis-guided-offer`, 189/189 Testdateien, 1.872/1.872 Tests, 90-Seiten-Build, produktiv 110/110, echter Dialog-/Volltext-/Kalkulationsklicktest, null Rückstände, leerer Prisma-Diff, WorkPilot PID `805996`, KlinikNavigator unverändert `398228`. Keine Prisma-Schemaänderung; `StoredFile`, privater S3-Speicher und Online-Anfragen-Invarianten bleiben erhalten.
- Rechnungspositions-Layout 2026-08-07: In normalen Rechnungsmasken liegt die interne Mitarbeiterzeit einer Leistungsposition als eigene vollbreite Zeile unter den kompakten Positionsfeldern, damit Mengen-, Einheiten-, Preis-, Rabatt- und Gesamtspalten nicht bis zur Unterkante der Abrechnungsgrundlage verlängert werden. Positionsbalken und Spaltenkopf verwenden dort die WorkPilot-Petrolpalette. Dauerläufer mit Stundenabrechnung behalten ihre gesonderte vollbreite Tages-/Stempelungsdarstellung unverändert; Berechnungs-, Zuordnungs-, PDF- und E-Rechnungslogik werden durch diese reine UI-Struktur nicht verändert.
- Arbeitszeit-/Pausenlogik 2026-08-08: `ProjectTimeEntry.durationMs` ist systemweit die kanonische Nettoarbeitszeit; `pauseMs` ist ein zusätzlicher Nachweis der Pause und darf für Auswertungen, Abwesenheitsprüfung oder Rechnungs-Vorprüfung nicht nochmals von `durationMs` abgezogen werden. Manuelle Zeiteinträge schlagen die Überschneidung mit dem beim Mitarbeiter hinterlegten Pausenfenster vor; eine kürzere Pause bleibt möglich, muss aber ausdrücklich bestätigt werden. Beim Beenden oder Wechseln einer Live-Stempelung wird eine fehlende geplante Pause ebenfalls nur transparent bestätigt, niemals automatisch von der tatsächlich gestempelten Nettozeit abgezogen. Der Zeitstatus vergleicht deshalb ausdrücklich Soll-Nettozeit mit Ist-Nettozeit und ist keine Aussage über Pünktlichkeit. Bestehende Zeiteinträge werden nicht rückwirkend verändert; keine Prisma-Schemaänderung.
- Belegmodal-Summen und PDF-Großansicht 2026-08-08: Rechnungs- und Angebotsmodale zeigen Rabatt, Netto, MwSt. und Brutto als gemeinsame kompakte Summenzeile; Brutto ist als Abschluss in der WorkPilot-Petrolfarbe hervorgehoben, aktive Beleg-Rabatte bleiben in einer zusätzlichen schmalen Informationszeile nachvollziehbar. Berechnungs-, Positions-, PDF-, Versand-, E-Rechnungs- und Stempelzuordnungslogik bleiben unverändert. Temporäre PDF-Vorschauen werden für `Groß öffnen` nicht mehr direkt als browserkritische Base64-Data-URL navigiert, sondern in eine kurzlebige lokale Blob-URL überführt; gespeicherte Beleg-PDFs verwenden weiterhin die geschützten WorkPilot-Routen.
- Stundenabrechnungs-Kundentext 2026-08-08: Der tageweise `Kundentext auf der Rechnung` in offenen Dauerläufer-Stundenabrechnungsentwürfen wird nach kurzer Eingabepause über einen eigenen organisations-, rollen-, entwurfs- und revisionsgebundenen Speicherpfad gesichert. Dieser Pfad ändert ausschließlich `InvoiceLine.hourlyBillingDetails`, erzeugt weder PDF noch Historieneintrag und speichert keine anderen offenen Maskenänderungen mit. Beim Wiederöffnen müssen die gespeicherten Tagesdetails in den Bearbeitungsentwurf übernommen werden; bei einem Speicherkonflikt darf kein fremder Stand überschrieben werden.
- Reaktionsschnelle Formulartexte 2026-08-08: Häufig bearbeitete Textfelder in Rechnungs-, Angebots-, Dokumentmail- und Mitarbeitereditoren verwenden eine gemeinsame lokale Eingabegrenze. Tastendrücke dürfen nicht pro Zeichen den gesamten `DashboardPage`-Berechnungsbaum auslösen; der fachliche Elternzustand wird nach 250 ms Schreibruhe sowie zwingend bei Blur beziehungsweise Enter aktualisiert. Externe Entwurfswechsel müssen den lokalen Wert ersetzen, Leerzeichen und Zeilenumbrüche bleiben unverändert, und Speicher-, Vorschau-, Mail- sowie Stundenabrechnungslogik erhalten vor einer nachfolgenden Aktion stets den letzten Feldwert. Diese UI-Entkopplung ändert weder Datenmodelle noch Auswertungsformeln.
- JARVIS vollständige Angebots- und Teamtermin-Dialoge 2026-08-21: Natürliche Angebotswünsche werden ohne zusätzliches Formular vollständig im Dialog bis zur revisions-, HMAC-, Sitzungs-, Rollen- und Organisations-gebundenen Vorschau geführt: Kunde per Volltext, ausschließlich offene Kundenprojekte, gegebenenfalls Bezugsangebot für Nachträge, Ausführungsmonat, aktive Katalogleistung per Volltext und stabil gebundener Auswahl-ID, Menge sowie weitere Positionen. Freie ungeprüfte Positionen bleiben gesperrt; erst der bestehende ausdrückliche Action-Center-Button darf genau einen Angebotsentwurf anlegen, niemals finalisieren oder versenden. Ein gewählter Vorschlag aus der Team-Verfügbarkeit übernimmt alle genannten Mitarbeiter und fragt anschließend Kunde sowie offenes Projekt ab, bevor die vorhandene projekttypgerechte Termin-/Terminwunschvorschau entsteht. `Abbrechen`, `Stopp` und `Verwerfen` beenden den gebundenen Ablauf ohne Fachobjekt. Die bounded Dialogzustände dürfen nicht als Rohdaten an den Browser gelangen; manipulierte oder veraltete IDs werden verworfen beziehungsweise organisationsgebunden neu validiert. Keine Prisma-Schemaänderung.
